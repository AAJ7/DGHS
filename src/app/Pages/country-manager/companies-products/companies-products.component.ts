...
@Component({
  ...
  styleUrls: ['./companies-products.component.scss']
})
export class CompaniesProductsComponent implements OnInit {
  ...
  constructor(private route:ActivatedRoute,private router:Router,private toast:NgToastService, private fb: FormBuilder,
    private CategoriesService:CategoriesService,
    private CategoriesCompaniesService:CategoriesCompaniesService,
    private datePipe: DatePipe,
    private CompaniesProductsService:CompaniesProductsService) { }
  ngOnInit(): void {
    this.company_id = this.route.snapshot.params['id'];
    this.products(this.company_id);
    this.CategoriesId();
  }
  products(company_id:any) {
    this.CompaniesProductsService.products(company_id).subscribe ({
      next:(response)=> {
  ...
  CreateForm(Create_Form:FormGroup) {
    if(Create_Form.valid) {
      ...
      this.CompaniesProductsService.create(formData).subscribe ({
        next:(response)=> {
          this.loader_create = false;
  ...
  Update_Form(Form:FormGroup) {
    if(Form.valid) {
      ...
      this.CompaniesProductsService.update(formData).subscribe ({
        next:(response)=> {
  ...
  deleteProduct() {
    this.CompaniesProductsService.delete(this.deleted_ProductId).subscribe ({
      next:(response)=> {
        this.loader_delete = false;
        this.toast.success({detail:"SUCCESS",summary:response?.message,duration:5000});
        this.products(this.CompanyId);
  ...
  toggleActivate() {
    ...
    this.CompaniesProductsService.activate(this.active_id , this.toggle).subscribe ({
      next:(response)=> {
        this.loader_activate = false;
