...
import { CooperationWithUsReasonService } from 'src/app/core/services/CooperationWithUsReason/cooperation-with-us-reason.service';
...
@Component({
  selector: 'app-cooperation-with-us-reason',
  templateUrl: './cooperation-with-us-reason.component.html',
  styleUrls: ['./cooperation-with-us-reason.component.scss']
})
export class CooperationWithUsReasonComponent implements OnInit {
  show_create:boolean = false;
  show_update:boolean = false;
  show_delete:boolean = false;
  show_stop:boolean = false;
  deleted_CooperationWithUsReasonId!:number;
  rows:any[] = [];
  temp:any[] = [];
  temps:any[] = [];
  img_path!:string;
  ...
  constructor(private CooperationWithUsReasonService:CooperationWithUsReasonService , private toast:NgToastService,private datePipe: DatePipe) {
  }

  ngOnInit(): void {
    this.CooperationWithUsReason();
  }
  CooperationWithUsReason() {
    this.CooperationWithUsReasonService.show().subscribe ({
      next:(response)=> {
  ...
  CreateForm(Create_Form:FormGroup) {
    if(Create_Form.valid) {
      ...
      this.CooperationWithUsReasonService.create(Create_Form.value).subscribe ({
        next:(response)=> {
  ...
  UpdateCooperationWithUsReason(row:any) {
  ...
  UpdateForm(Update_Form:FormGroup) {
    if(Update_Form.valid) {
      ...
      this.CooperationWithUsReasonService.update(Update_Form.value).subscribe ({
        next:(response)=> {
          this.loader_update = false;
          if(!response?.status && response?.message?.img_path) {
            this.toast.error({detail:"ERROR",summary:"The image must be type of png , jpg , jpeg",duration:5000});
  ...
  GetCooperationWithUsReasonId(id:number) {
    this.deleted_CooperationWithUsReasonId = id;
    this.show_delete = true;
    this.loader_delete = false;
  }
  deleteCooperationWithUsReason() {
    this.loader_delete = true;
    this.CooperationWithUsReasonService.delete(this.deleted_CooperationWithUsReasonId).subscribe ({
      next:(response)=> {
        this.loader_delete = false;
        if(response?.status) {
          this.toast.success({detail:"SUCCESS",summary:response?.message,duration:5000});
